...
class LangchainChunkingRepository(ChunkingRepository):
    ...
    def combine_sentences(self, documents:list[DocumentValue], buffer_size:int) -> list[str]:
        ...
        for i, sentence in enumerate(sentences):
            combined = " ".join(sentences[max(0, i - buffer_size): i + buffer_size + 1])
    ...
    def semantic_chunking(self, embeddings_sentence:list[EmbeddingSentence],breakpoint_percentile:float) -> list[ChunkValue]:
        ...
        indices_above_thresh = [i for i, x in enumerate(similarities) if x > breakpoint_threshold]
        ...
        if start_index < len(embeddings_sentence):
            combined_text = ' '.join([d.sentence for d in embeddings_sentence[start_index:]])
            chunks.append(ChunkValue(content=combined_text, metadata={}))
