...
class ChankingUseCase:
    ...
    """
    def __init__(self, chunking_repository:ChunkingRepository,embeding_repository:EmbeddingRepository):
    ...
    def semantic_chunking(self, documents:list[DocumentValue],buffer_size:int=1,breakpoint_percentile:float=95) -> list[ChunkValue]:
        combined_sentences = self.chunking_repository.combine_sentences(documents, buffer_size)
        print(f'combined_sentences: {combined_sentences}')
        embeddings = self.embeding_repository.embed_list(combined_sentences)
        print(f'embeddings: {embeddings}')
        embeddings_sentence=[EmbeddingSentence(combined_sentences[i],embeddings[i])  for i in range(len(combined_sentences))]
        print(f'embeddings_sentence: {embeddings_sentence}')
        chunks = self.chunking_repository.semantic_chunking(embeddings_sentence,breakpoint_percentile)
        return chunks
